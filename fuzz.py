...
DEFAULT_BASE_URL = input("Enter your URL:")
...
class AsyncURLFuzzerPathTraversal(object):
    """
        ...
        try:
            ...
            for t in tasks:
                ...
                if self._is_valid_status_code(status_code):
                     ...
                     #print("Valid status code is =")
                     #print(self._active_paths_status_codes[path])
                     print(full_path)
...
class AsyncURLFuzzerSimpleTest(object):
    """
    An asynchronous http(s) website endpoint locator.
    Discovers active endpoints in websites, based on a list of common URLS.
    """
...
##### Directry Traversal###########
class AsyncURLFuzzerDirectryTraversal(object):
        """
        An asynchronous http(s) website endpoint locator.
        Discovers active endpoints in websites, based on a list of common URLS.
        """
        ...
        def _save_output_log(self):
            ...
            Saves the results to an output file.
            """
            full_status_codes = {'/'.join([self._base_url, p]): code for p, code in
                                 self._active_paths_status_codes.items()}
            output_lines = ['{0} : {1}'.format(path, code) for path, code in full_status_codes.items()]
            if 1 >= len(output_lines):
                self._logger.warning(
                    'There were no discovered endpoints. consider using a different file from "words_list" directory')
            self._logger.info(
                'The following endpoints are active:{0}{1}'.format(os.linesep, os.linesep.join(output_lines)))
            with open(self._output_file_path, 'a+') as output_file:
                output_lines.sort()
                output_file.write(os.linesep.join(output_lines))
            self._logger.info('The endpoints were exported to "{0}"'.format(self._output_file_path))
...
def PathTraversal():
    ...
    parser.add_option('-l', '--list', dest='list_file',
                      help='A file containing the paths to check (separated with lines).',
    ...
    if (os.path.isdir(base_url) or os.path.isfile(base_url)):
        ...
        for u in FilesFactory.urls:
            fuzzer = AsyncURLFuzzerPathTraversal(u, list_file)
            fuzzer.start()

    else:
        fuzzer = AsyncURLFuzzerPathTraversal(base_url, list_file)
        fuzzer.start()
...
def SimpleTest():
    ...
    if (os.path.isdir(base_url) or os.path.isfile(base_url)):
        FilesFactory(base_url)
        for u in FilesFactory.urls:
            fuzzer = AsyncURLFuzzerSimpleTest(u, list_file)
            fuzzer.start()

    else:
      fuzzer = AsyncURLFuzzerSimpleTest(base_url, list_file)
      fuzzer.start()
...
def DirectryTraversal():
    ...
    if (os.path.isdir(base_url) or os.path.isfile(base_url)):
        FilesFactory(base_url)
        for u in FilesFactory.urls:
            fuzzer = AsyncURLFuzzerDirectryTraversal(u, list_file)
            fuzzer.start()

    else:
        fuzzer = AsyncURLFuzzerDirectryTraversal(base_url, list_file)
        fuzzer.start()
